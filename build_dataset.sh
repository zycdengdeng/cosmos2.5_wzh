...
# ============================================================================
# Cosmos Transfer2.5 数据集构建脚本
# 功能：将 guidance 和 GT 数据组织成官方训练格式
# ============================================================================
...
# 目标数据集路径
DATASET_BASE="/mnt/zihanw/cosmos-transfer2.5/datasets/self_Data_Training"
...
copy_videos() {
    ...
    for seq in "${SEQUENCES[@]}"; do
        ...
        # 构建源路径
        guidance_dir="${GUIDANCE_BASE}/${seq}_90frames_1280x720/color"
        gt_dir="${GT_BASE}/${seq}_GT_90frames_1280x720"

        # 检查源目录是否存在
        if [ ! -d "$guidance_dir" ]; then
            echo "⚠ 警告: Guidance 目录不存在: $guidance_dir"
            continue
        fi
        ...
        for cam_src in "${!CAMERA_MAP[@]}"; do
            ...
            # 源文件名
            guidance_file="${guidance_dir}/${cam_src}_color.mp4"
            ...
            # 复制 guidance (control input)
            if [ -f "$guidance_file" ]; then
                cp "$guidance_file" "$control_dst"
                echo "  ✓ 复制: ${cam_src}_color.mp4 -> control_input/${cam_dst}/${seq}.mp4"
                success_count=$((success_count + 1))
            else
                echo "  ✗ 缺失: $guidance_file"
            fi
...
{
    "caption": "A driving scene captured from a vehicle's front wide camera",
    "sequence_id": "${seq}",
...
generate_summary() {
    ...
    for cam_folder in "${CAMERA_MAP[@]}"; do
        ...
        echo "  ${cam_folder}:"
        echo "    - control_input: ${control_count} 文件"
        echo "    - videos: ${video_count} 文件"
    done
...
- **control_input_hdmap_bbox/**: 包含彩色点云投影生成的 guidance 视频
...
1. **更新 Captions**: 编辑 `captions/ftheta_camera_front_wide_120fov/*.json` 文件，添加实际的场景描述
2. **验证数据**: 使用训练脚本验证数据集格式是否正确
3. **开始训练**: 参考 `docs/post-training_auto_multiview.md` 进行模型训练
...
main() {
    echo ""
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║   Cosmos Transfer2.5 数据集构建脚本                            ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo ""
